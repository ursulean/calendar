...
export function eventSourceFunction(calendarObjects, calendar, start, end, timezone) {
	...
	for (const calendarObject of calendarObjects) {
		...
		for (const object of allObjectsInTimeRange) {
			const fcEvent = vObjectSourceFunction(calendarObject, object, calendar, timezone)
			if (fcEvent !== undefined) { fcEvents.push(fcEvent) }
		}
	}
	return fcEvents
}

export function vObjectSourceFunction(calendarObject, object, calendar, timezone) {
	const classNames = []
